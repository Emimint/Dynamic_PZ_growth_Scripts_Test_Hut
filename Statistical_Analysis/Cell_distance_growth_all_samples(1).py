...
plot_type = "regplot" # lineplot or regplot
zone_selection = "boundary" # APZ, boundary, primordium or blank.
...
x_value = "Speed" # Timepoint,Speed, Relative_deformation, Zone, Growth, Cell_distance, BF_value
y_value = "Growth" # Timepoint,Speed, Relative_deformation, Zone, Growth, Cell_distance, BF_value
hue_value = "" # Primordium, Timepoint,Speed, Relative_deformation, Zone, Growth, Cell_distance, Sample_Primordium, Sample_Primordium_Zone, BF_value
option = "full" #for complete, full file (file with both cell distance and growth information - i.e. option = "full") or minimalist file (file with growth information - i.e. option is blank).
...
if(plot_type == "lineplot"):
    ...
    if(hue_value):
        # ax = sns.lineplot(data=In_use_df, x=x_value, y=y_value, hue=hue_value,legend= "full", ci = "sd")
        ...
        ax = sns.lineplot(data=In_use_df, x=x_value, y=y_value, hue="Zone",style="Zone",palette=['darkblue','lightgreen','coral'], markers=True, dashes=[(2,0),(1,1),(1,2)],legend= "full", ci = "sd")
        # ax = sns.lineplot(data=In_use_df, x=x_value, y=y_value, hue="Zone",style="Zone",palette=['lightblue','coral','yellow'], markers=True, dashes=[(2,0),(1,1),(1,2)],legend= "full", ci = "sd")
    else:
...
if(plot_type == "regplot"):
    fig = plt.gcf()
    fig.set_size_inches(12, 9)
    # ax = sns.scatterplot(data=In_use_df, x=x_value, y=y_value,color = colour, legend=None)
    ax = sns.regplot(x=x_value, y=y_value, data=In_use_df, robust = True , color = colour,ci=None)
    # ax = sns.lmplot(x=x_value, y=y_value, data=In_use_df, robust=True, ci=None,scatter_kws={"s": 80})
